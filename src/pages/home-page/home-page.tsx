...
import { LabelWrapper, ResetWrapper, RotateWrapper, Section } from "./home-page.style";
...
export default function HomePage() {
  ...
  const rotateWrapperRef = React.useRef<HTMLDivElement>(null);
  const resetWrapperRef = React.useRef<HTMLDivElement>(null);
  ...
  React.useEffect(() => {
    ...
    const rotateWrapper = rotateWrapperRef.current;
    const label = labelRef.current;
    if (menuContainer === null || rotateWrapper === null || label === null) return;
    if (!mounted) return;
    if (mainMenuOpen) return;
    rotateWrapper.style.animationName = "showElement";
    rotateWrapper.style.opacity = "0";
